...
void ROIAlignRotated::executeImpl() {
    ...
    ov::reference::roi_align<T, ov::reference::roi_policy::ROIAlignRotatedOpDefPolicy>(
        ...
        ov::Shape{getSrcMemoryAtPort(0)->getStaticDims().to_vec()},
        ov::Shape{getSrcMemoryAtPort(1)->getStaticDims().to_vec()},
        ov::Shape{getSrcMemoryAtPort(2)->getStaticDims().to_vec()},
        ov::Shape{getDstMemoryAtPort(0)->getStaticDims().to_vec()},
        pooledH,
        pooledW,
        samplingRatio,
        spatialScale,
