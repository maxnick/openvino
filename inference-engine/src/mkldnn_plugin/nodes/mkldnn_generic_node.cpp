...
void MKLDNNGenericNode::execLayer() {
    ...
    for (size_t i = 0; i < getParentEdges().size(); i++) {
        auto inputBlob = MemoryDescUtils::convertMemToBlob(getParentEdgeAt(i)->getMemory());
    ...
    for (size_t i = 0; i < outputShapes.size(); i++) {
        if (isDynBatch) {
            ...
            outputs.push_back(MemoryDescUtils::convertMemToBlob(getChildEdgesAtPort(i)[0]->getMemory()));
        }
    }
    InferenceEngine::ResponseDesc resp;
