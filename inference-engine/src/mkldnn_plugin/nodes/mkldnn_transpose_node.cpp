...
void MKLDNNTransposeNode::initSupportedPrimitiveDescriptors() {
    ...
    config.inConfs[1].desc = MKLDNNPlugin::make_unique<DnnlMemoryDesc>(getInputShapeAtPort(1).getStaticDims(), inputOrderDataType,
                                                                         memory::format_tag::x);
    if (getInputShapeAtPort(0).getRank() == 4) {
        config.inConfs[0].desc = MKLDNNPlugin::make_unique<DnnlMemoryDesc>(getInputShapeAtPort(0).getStaticDims(), inputDataType,
                                                                             memory::format_tag::nchw);
        config.outConfs[0].desc = MKLDNNPlugin::make_unique<DnnlMemoryDesc>(getOutputShapeAtPort(0).getStaticDims(), outputDataType,
                                                                              memory::format_tag::nchw);
