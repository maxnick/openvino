...
void MKLDNNROIPoolingNode::initSupportedPrimitiveDescriptors() {
    ...
    config.inConfs[0].desc = MKLDNNPlugin::make_unique<OnednnMemoryDesc>(getParentEdgeAt(0)->getShape().getStaticDims(), dataType, format);
    config.inConfs[1].desc = MKLDNNPlugin::make_unique<OnednnMemoryDesc>(getParentEdgeAt(1)->getShape().getStaticDims(), dataType, memory::format_tag::nc);
    config.outConfs[0].desc = MKLDNNPlugin::make_unique<OnednnMemoryDesc>(getChildEdgeAt(0)->getShape().getStaticDims(), dataType, format);
