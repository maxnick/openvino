...
void MKLDNNStridedSliceNode::initSupportedPrimitiveDescriptors() {
    ...
    for (auto itr = range.first; itr != range.second; ++itr) {
        ...
        config.inConfs[BEGIN_ID].desc = MKLDNNPlugin::make_unique<OnednnMemoryDesc>(getParentEdgeAt(BEGIN_ID)->getShape().getStaticDims(), beginDataType,
                                                                      mkldnn::memory::format_tag::x);
        config.inConfs[END_ID].desc = MKLDNNPlugin::make_unique<OnednnMemoryDesc>(getParentEdgeAt(END_ID)->getShape().getStaticDims(), endDataType,
                                                                    mkldnn::memory::format_tag::x);
        if (hasStrides)
            config.inConfs[STRIDE_ID].desc = MKLDNNPlugin::make_unique<OnednnMemoryDesc>(getParentEdgeAt(STRIDE_ID)->getShape().getStaticDims(),
